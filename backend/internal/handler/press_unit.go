...
type PressUnitHandler struct{ service service.PressUnitService }

func NewPressUnitHandler(s service.PressUnitService) *PressUnitHandler {
	return &PressUnitHandler{service: s}
}

func (h *PressUnitHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/presses")
	resource.GET("", h.list)
...
func (h *PressUnitHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *PressUnitHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *PressUnitHandler) create(c *gin.Context) {
	var input dto.CreatePressUnit
...
func (h *PressUnitHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdatePressUnit
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *PressUnitHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *PressUnitHandler) remove(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
