...
type SystemHandler struct {
	...
	pressUnit       service.PressUnitService
	printRun        service.PrintRunService
	colorProof      service.ColorProofService
	releaseDecision service.ReleaseDecisionService
	db              *gorm.DB
	redis           *redis.Client
}

func NewSystemHandler(security service.SecurityService, pressUnit service.PressUnitService, printRun service.PrintRunService, colorProof service.ColorProofService, releaseDecision service.ReleaseDecisionService, db *gorm.DB, redisClient *redis.Client) *SystemHandler {
	return &SystemHandler{security: security, pressUnit: pressUnit, printRun: printRun, colorProof: colorProof, releaseDecision: releaseDecision, db: db, redis: redisClient}
}
...
func (h *SystemHandler) Overview(c *gin.Context) {
	...
	pressUnitCounts, err := h.pressUnit.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["presses"] = pressUnitCounts

	printRunCounts, err := h.printRun.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["runs"] = printRunCounts

	colorProofCounts, err := h.colorProof.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["proofs"] = colorProofCounts

	releaseDecisionCounts, err := h.releaseDecision.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["release"] = releaseDecisionCounts
