...
type ReleaseDecisionHandler struct {
	service service.ReleaseDecisionService
}

func NewReleaseDecisionHandler(s service.ReleaseDecisionService) *ReleaseDecisionHandler {
	return &ReleaseDecisionHandler{service: s}
}

func (h *ReleaseDecisionHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/release")
	resource.GET("", h.list)
...
func (h *ReleaseDecisionHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *ReleaseDecisionHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *ReleaseDecisionHandler) create(c *gin.Context) {
	var input dto.CreateReleaseDecision
...
func (h *ReleaseDecisionHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateReleaseDecision
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *ReleaseDecisionHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *ReleaseDecisionHandler) remove(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
